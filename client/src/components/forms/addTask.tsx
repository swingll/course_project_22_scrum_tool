...
export function AddTask({ storyId, members, status, className, loading, setLoading }: any) {
  ...
  let userContent;
  if (!members || !members.length || userLoading)
        ...
        <ModalBody>
          ...
          <FormGroup>
            ...
            <Input type="select" name="contributors" id="contributors" onChange={(e) => setContributors(e.target.value)}>
              {(members && members.length && !userLoading) ? <option value="">Choose</option> : <></>}
              {userContent}
            </Input>
